...
#---
def outputnew( s ):
    if print_pywikibot[1] and 'test' in sys_argv:
        pywikibot.output( s )
#---
# ref_complite = re.compile(r'(<ref([^>]*)>[^<>]+</ref>)')
# ref_short    = re.compile(r'(<ref([^>]*)\/\s*>)')
#---
# ref_complite = re.compile(r'(<ref\s*name\s*\=\s*\"*([^>]*)\"*\s*>[^<>]+</ref>)')
#---
# ref_complite = re.compile(r'(<ref(?:\s*name\s*\=\s*(?:\"*([^>]*)\s*\"*|([^>]*))\s*|([^>]*))>[^<>]+</ref>)')
#---
ref_complite = re.compile(r'(<ref\s*name\s*\=*\s*\"*([^>]*)\"*\s*>[^<>]+</ref>)')
