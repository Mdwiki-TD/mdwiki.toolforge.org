...
# ---
sect_file = f'{Dir}/lists/who_links.json'
...
def dump_secs_links(who_links):
    # ---
    global sect_file
    # ---
    if who_links != {}:
        printe.output(f'<<lightyellow>> who_links(): lenth: {len(who_links.keys())}')
        json.dump(who_links, codecs.open(sect_file, 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
...
def get_section_links(new=False):
    ...
    if new or len(old) == 0:
        ...
        who_links = bot.SectionsToLinks

        dump_secs_links(who_links)
    else:
        who_links = old
    # ---
    return who_links
