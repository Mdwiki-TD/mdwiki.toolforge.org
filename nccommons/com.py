...
python3 pwb.py ncc/com -limitall:50000 -files:200
...
def Log_to_wiki(family='nccommons', lang = "www" ):
    #---
    print( "nccommons/com.py: log to %s.%s.org user:%s" % (lang,family , r2_params['lgname'] )  )
    SS["family"] = family
    SS["lang"] = lang
    ...
    if SS:
        ...
        print( "com.py: Can't log in . ")
        return False
    #---
    if r22.json()['login']['result'] != 'Success':
        print(r22.json()['login']['reason'])
    else:
...
def post( params ):
    ...
    try:
        ...
        print("error when json loads")
    #---
    jj = {}
    #---
    try:
        jj = json.loads(r4.text)
    except:
        print("error when json loads")
    #---
    return jj
...
def Get_All_pages_A( start , namespace = "0" , limit = "max" , apfilterredir = '',apcontinue='' ):
    ...
    if json1 : 
        ...
        print( "<<lightpurple>> --- Get_All_pages_A : find %d pages." % len(newss) )
    #---
    return json1
#---
def Get_All_pages( start , namespace = "0" , limit = "max" , apfilterredir = '' , limit_all = 0 ):
    ...
    while Apcontinue2[1] != '' : 
        ...
        numb += 1
        #---
        print( 'Get_All_pages_A %d, Apcontinue2[1]:"%s"..' % ( numb , Apcontinue2[1] ) )
        #---
        ...
        if limit_all > 0 and len(Main_table) > limit_all : 
            Apcontinue2[1] = '' 
            print( "<<lightgreen>> limit_all > len(Main_table) " )
            break
        #---
    #---
    if numb > 0 and Apcontinue2[1] == '' : 
        print( "<<lightgreen>> Apcontinue2[1] == '' " )
    #---
...
def create_Page(text, title):
    ...
    if not Save_all[1] and ("ask" in sys.argv and not "save" in sys.argv ) :
        ...
        if sa.strip() == "a":
            print('<<lightgreen>> ---------------------------------------------' )
            print('<<lightgreen>> nccommons.py create_Page save all without asking.' )
            print('<<lightgreen>> ---------------------------------------------' )
            Save_all[1] = True
        #---
    #---
    ...
    #---
    if success:
        print('<<lightgreen>>** true ..  %s : [[%s]] '   % (SS["family"] , title) )
        ...
        return False
    #---
    print( "<<lightred>> end of create_Page def return False title:(%s)" % title )
    #---
    return False
#---
ns_0_pages = Get_All_pages( '' , limit = "max", namespace = "0" )
#---
files_all = 0
limitall = 0
#---
for arg in sys.argv:
    ...
    #---
    if arg == '-files':         files_all = int(value)
    #---
#---
...
def work():
    ...
    for title in all_files:
        ...
        if mate : 
            ...
            nomatch += 1
            print('<<lightblue>> title:%d/\t%s.' % (nomatch,title))
    #---
    ...
    for le,x in da:
        ...
        if le > files_all :
            ...
            if not x in ns_0_pages:
                ...
                print('<<lightred>> page:%s already in ns_0_pages.' % x )
#---
