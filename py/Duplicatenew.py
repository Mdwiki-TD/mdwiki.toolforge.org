        ...
        if namefound:
            ...
            if Group == None:
                if quote == '' or quote == None :
                    ...
                    if name2 and name2 != "":
                        name = name2
                        pywikibot.output("get the name again:%s" % name )
            #---
...
def DuplicateReferences(text):
    ...
    for Match in REFS.finditer(text):
        ...
        groupdict[content] = v
    #---
    pywikibot.output('found %d in found_ref_names.' % len(found_ref_names) )
    #---
...
x <ref name="autogen4">{{cite journal |last1=Bingham |first1=G |last2=Rentea |first2=RM |title=Posterior Urethral Valve |date=January 2020 |pmid=32809716}}</ref>

ing  <ref name="autogen4">{{Cite journal|last2=Rentea |first2=RM |title=Posterior Urethral Valve |date=January 2020 |pmid=32809716|last=Bingham|first=G}}</ref>
    """
    #---
    tet = """
{{Infobox medical condition (new)
| risksx = <ref name=Fer2016>t1</ref>
| risks = <ref name=Fer2016/>
}}
'''PFPS'''<ref name=Fer2016>t1</ref>
