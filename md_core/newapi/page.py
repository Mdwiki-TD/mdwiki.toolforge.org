...
class Login():
	...
	def Log_to_wiki(self):
		...
		#---
		printe.output( "newapi/page.py: log to %s:%s. user:%s" % (self.lang, self.family, self.username )  )
		#---
...
class MainPage():
	...
	def can_edit(self, script=''):
		if self.text == '' : self.text = self.get_text()
		#---
		self.can_be_edit = Trur#botEdit.botMayEdit( False, text= self.text, title_page=self.title, botjob=script)
		#---
		return self.can_be_edit
